...
const StoreOrderSummary: FC<Props> = ({ group }) => {
  const paymentDetails = group.order.payementDetails;
  const paymentStatus = group.order.payementStatus as PaymentStatus;
  const shippingAddress = group.order.shippingAddress;

  const { address1, firstName, lastName, phone, willaya, user } =
    shippingAddress;
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 py-4 border-gray-100 mb-6">
          ...
          <div className="grid grid-cols-2">
            ...
            <div>
              ...
              <h6 className="font-semibold text-lg leading-9">
                {paymentDetails?.payementInentId || "-"}
              </h6>
            </div>
          </div>
          {/* Address */}
