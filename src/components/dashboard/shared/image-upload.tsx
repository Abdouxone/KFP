  ...
  if (type === "profile") {
      ...
      <div className="relative rounded-full w-52 h-52  bg-gray-200 border-2 border-white shadow-2xl overflow-visible">
        ...
        <CldUploadWidget uploadPreset="kfp_test" onSuccess={onUpload}>
          {({ open }) => {
            const onClick = () => {
              setTimeout(() => {
                open?.(); // safer with optional chaining
              }, 10); // small delay allows browser to treat it as user interaction
            };
    ...
    return (
      <div>
        ...
        <CldUploadWidget onSuccess={onUpload} uploadPreset="kfp_test">
          {({ open }) => {
            const onClick = () => {
              open();
            };
